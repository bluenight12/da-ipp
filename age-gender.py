...
class AgeGenderPrediction:          # AgeGenderPrediction 클래스는 성별 및 연령 예측을 담당합니다. 이 클래스는 OpenVINO 모델을 사용하여 감지된 얼굴의 성별과 연령을 예측합니다.
    def __init__(self, model_xml, model_bin, device="CPU"):
        ...
        self.compiled_model = self.core.compile_model(          
            model=self.model, device_name=self.device.value)        # 모델을 실행할 디바이스에 맞게 최적화합니다.
        self.input_keys = self.compiled_model.input(0)              # 모델의 입력 키를 가져옵니다.
        self.age_output_key = self.compiled_model.output(0)         # 각각 나이와 성별의 출력 키를 가져옵니다.
...
# face_detector = FaceDetection(model_xml='model/face-detection-retail-0005/face-detection-retail-0005.xml',
#                               model_bin='model/face-detection-retail-0005/face-detection-retail-0005..bin')
# age_gender_predictor = AgeGenderPrediction(model_xml='model/age-gender-recognition-retail-0013/age-gender-recognition-retail-0013.xml',
#                                            model_bin='model/age-gender-recognition-retail-0013/age-gender-recognition-retail-0013.bin')
...
#     a = face_detector.run(frame)

#     # print(a)
    
#     b , c = age_gender_predictor.run(a)
#     print(b,c)
