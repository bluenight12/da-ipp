...
class RUN:
    ...
    def take_photo(self):
        ...
        while 1:
            ...
            key = cv2.waitKey(10)
            #추후에 사진 저장 변경하고 순서대로 실행
            if key == ord('q'): break
            elif key == ord('s'):
                cv2.imwrite(f'{os.path.realpath(__file__)}/detection/predict.png', frame)
            elif key == ord('a'): 
                cv2.imwrite(f'inputs/img/img001.png', frame)
    ...
    def set_background(self):
        ...
        u2net_run.infer(self.u2net, 'results/test/final', 'Data_preprocessing/test_mask')
