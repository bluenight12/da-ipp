    ...
    with open(input_file_path, 'r') as english_file:
        ...
        with open(output_file_path, 'w') as clothes_file:
            ...
            for line in english_file:                    
                ...
                clothes_file.write(' '.join([word for word in words if word in ['red', 'black', 'blue', '-shirt', 'green', 'man to man', 'knit', 'white', 'long sleeve']]) + '\n')

# 손 봐야 할 부분이 많음
# extract_clothes(f"{os.path.dirname(os.path.realpath(__file__))}/english.txt", f"{os.path.dirname(os.path.realpath(__file__))}/clothes.txt")
